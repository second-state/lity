...
public:
  ...
  static EVMVersion lity() { return {Version::Lity}; }
	...
	{
		for (auto const& v: {homestead(), tangerineWhistle(), spuriousDragon(), byzantium(), lity(), constantinople(), petersburg(), istanbul(), berlin()})
			if (_version == v.name())
				return v;
		return {};
	}
...
private:
	...
	enum class Version { Homestead, TangerineWhistle, SpuriousDragon, Byzantium, Constantinople, Petersburg, Istanbul, Berlin, Lity };

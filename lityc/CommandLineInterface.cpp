...
Allowed options)",
	...
	desc.add_options()
		...
		(
			...
			"Select desired EVM version. Either homestead, tangerineWhistle, spuriousDragon, byzantium, lity (default) or constantinople."
		)
	...
	if (m_args.count(g_argContractStandard))
	{
		...
		if (standards.size() > 1)
		{
			serr() << "More than one standard given to --contract-standard" << endl;
			return false;
		}
	}
...
bool CommandLineInterface::processInput()
{
	...
	if (m_args.count(g_strContractStandard))
	{
		...
		{
			serr() << "Unknown contract standard given to --contract-standard: " << contractStandardStr << endl;
			return false;
		}
...
void CommandLineInterface::executeOyente()
{
	...
	for (string const& dir: dirs)
	{
		...
		{
			sout() << endl << "======= Oyente Analysis Report =======" << endl;
			if (m_args.count(g_argInputFile))
			{

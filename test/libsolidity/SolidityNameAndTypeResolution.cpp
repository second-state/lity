...
BOOST_AUTO_TEST_CASE(address_call_full_return_type)
{
	...
	vector<pair<Error::Type, std::string>> expectations(vector<pair<Error::Type, std::string>>{
		{Error::Type::Warning, "The use of low level \"call\" should be avoided"}
	});

	if (!dev::test::Options::get().evmVersion().supportsReturndata())
		expectations.emplace_back(make_pair(
			Error::Type::TypeError,
			std::string("Type inaccessible dynamic type is not implicitly convertible to expected type bytes memory.")
		));
	CHECK_ALLOW_MULTI(sourceCode, expectations);
}
